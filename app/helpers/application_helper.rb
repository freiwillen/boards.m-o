module ApplicationHelper
  ...
  def point_with_boards_json p
    "{coords:[#{p.coords}],code:'#{p.boards.map do |b| link_to_unless_current(b.code, board_by_code_path(b.code)) end.join(', ')}',address:'#{p.address.gsub("'","’")}',btype:'сосед',multi:'1',city:'#{p.city.name}',region:'#{p.region.name}'}"
  end
